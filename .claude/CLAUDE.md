...
A **PM automation toolkit** built around 5 specialized Copilot agents for the Brain/AIOps platform at Microsoft. This is not a compiled application — it's an agent-based system where the primary artifacts are system prompts, knowledge files, and lightweight tooling scripts.
...
1. **Agents** (`agents/`) — 4 Copilot agents registered via `copilot.json`:
   ...
   - **User Research** (`user-research/`) — Research strategist for any type of customer/user research. Invokes `skills/interview-analysis.skill.md` for multi-transcript analysis with AI guardrails (quote verification, context loading, few-shot calibration, contradiction checks). Also handles research planning, JTBD, methodology selection, and spec integration.
...
3. **Tools & Skills** (`tools/`, `skills/`, `agents/*/knowledge/`) — Shared utilities (doc extraction, action items, ADO sync, end-of-day, doc classification), shared skills (interview analysis, md-to-pdf), and agent-specific reference material (templates, content samples, checklists).
...
- **Team structure and domain model** are documented in `team-knowledge/` — agents discover domain files dynamically. Load all `.md` files in `team-knowledge/` root for domain context.
...
- **Check `skills/*.skill.md`** for predefined workflows before performing common tasks.
