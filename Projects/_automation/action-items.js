 ...
 * 
 * Appends new items to action-items.md for tracking over time.
 * 
 * Usage:
...
const TRACKER_FILE = path.join(__dirname, 'task-tracker.md');
...
function loadPrompts() {
    ...
    // Parse sections by finding ## headings
    // Split on --- (with flexible whitespace)
    const sections = content.split(/\r?\n---\r?\n/);
    for (const section of sections) {
        // Match ## heading anywhere in the section, capture everything after it
        // Allow for single newline after heading too
...
async function gatherActionItems(client, dateRange, prompts) {
    ...
    // Helper to substitute date placeholder in prompts
    const fillPrompt = (template) => template.replace(/\{\{dateDescription\}\}/g, dateDescription);
    
    // Query 1: Teams Meetings action items
    if (prompts.meetings) {
        ...
        if (meetingsResult) {
            ...
            if (items.length > 0) {
                console.log(`    ✅ Found ${items.length} meeting action items`);
                allItems.push(...items);
            } else {
                console.log(`    ℹ️  No meeting action items found`);
            }
        }
    }
    
    // Query 2: Teams Chats action items
    if (prompts.chats) {
        ...
        if (chatsResult) {
            ...
            if (items.length > 0) {
                console.log(`    ✅ Found ${items.length} chat action items`);
                allItems.push(...items);
            } else {
                console.log(`    ℹ️  No chat action items found`);
            }
        }
    }
    
    // Query 3: Outlook Emails action items
    if (prompts.emails) {
        ...
        if (emailsResult) {
            ...
            if (items.length > 0) {
                console.log(`    ✅ Found ${items.length} email action items`);
                allItems.push(...items);
            } else {
                console.log(`    ℹ️  No email action items found`);
            }
        }
    }
    
    // Query 4: Calendar - upcoming meetings to prep for
    if (prompts.calendar) {
        ...
        if (calendarResult) {
            ...
            if (items.length > 0) {
                console.log(`    ✅ Found ${items.length} prep items`);
                allItems.push(...items);
            } else {
                console.log(`    ℹ️  No meeting prep items found`);
            }
        }
    }
...
function parseWorkIQItems(response, source, dateAdded) {
    try {
        ...
        if (!jsonMatch) {
            // If no JSON array found, try to parse the whole thing
            return [];
        }
...
function saveActionItems(content, dateRange) {
    const outputDir = path.join(PROJECTS_DIR, '_automation', 'output');
    ...
    const filename = dateRange.start === dateRange.end 
        ? `action-items-${dateRange.end}.md`
        : `action-items-${dateRange.start}-to-${dateRange.end}.md`;
...
async function main() {
    ...
    try {
        ...
        if (newItems.length === 0) {
            ...
            // Append to tracker file unless --no-append
            if (!NO_APPEND) {
                const { activeItems, completedItems } = readTracker();
                
                // Move checked items to completed
                const stillActive = [];
