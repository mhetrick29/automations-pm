#!/usr/bin/env node
/**
 * Classify My Work (Work IQ Edition)
 * 
 * Uses Work IQ to find documents edited/created today, then:
 * - OneDrive docs: Move to appropriate project folder
...
const ONEDRIVE_ROOT = path.dirname(PROJECTS_DIR);
...
// Load all projects with their manifests
...
// Get documents edited today from Work IQ
...
// Classify a document to a project using Work IQ
...
// Parse the document list from Work IQ
function parseDocList(docListText) {
    const docs = [];
    
    // Extract URLs from markdown citations like [1](https://...)
    ...
    // Parse the pipe-delimited lines
    ...
    for (const line of lines) {
        ...
        if (parts.length >= 4) {
            ...
            // If URL says "not returned", look for a citation reference in the description
            ...
            // Determine if OneDrive based on URL
            ...
            docs.push({
                filename: parts[0].replace(/^\*\*|\*\*$/g, ''), // Remove markdown bold
                location: location,
                url: url,
                description: parts[3].replace(/\[\d+\]\([^\)]+\)/g, '').trim() // Remove citations
            });
        }
    }
...
${DRY_RUN ? '⚠️  DRY RUN MODE - No changes will be made\n' : ''}`);
    ...
    try {
        ...
        for (const doc of docs) {
            ...
            if (isOneDrive) {
                oneDriveCount++;
                // For OneDrive docs, we'd move them - but this requires knowing the local path
                // Work IQ gives us URLs, not local paths. We'll add as link for now.
                console.log(`   📁 OneDrive doc - adding link to manifest`);
                addLinkToManifest(project, doc.filename, doc.url, doc.description);
            } else {
                externalCount++;
                // External doc - add link to manifest
        ...
        if (unknownDocs.length > 0) {
            ...
            // Create _needs-sorting manifest entry
